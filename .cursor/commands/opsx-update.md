...
4. **Read and reconcile**
   ...
   - Revise only files that already exist (`existingOutputPaths`). Do NOT create artifacts that don't exist yet, and do NOT invent new files under a glob artifact - note them and point the user to `/opsx:apply` to create them.
...
6. **Point to the next step (guidance only - NEVER act on it)**
   - Artifacts still missing -> suggest `/opsx:apply` to create them.
...
- Anything deferred to `/opsx:apply` (not-yet-created artifacts or files)
...
- Do not advance the build frontier: no new artifacts, no new files under glob artifacts - that is `/opsx:apply`'s job.
