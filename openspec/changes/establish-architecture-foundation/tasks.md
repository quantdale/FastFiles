...
- [x] 7.7 Fuzz the frame parser (oversized frames, record-count mismatches, out-of-range length-prefixed fields) against both the service and the engine — **implemented**: `tests/protocol/test_fuzz.cpp` (seeded, reproducible, wired into `ctest` as `ffprotocol_fuzz_tests`) fuzzes `ValidateFrame`, `ValidateUiFrame`, `ParseMftBatch`, `ParseUsnDeltaBatch`, and `ParseSnapshot` — the shared parsing code both processes use. **Unblocked and closed** via the `autonomous-runtime-verification` harness: `ffprotocol_fuzz_tests` now actually runs (20,000 iterations/fuzzer, seed `0xF457F17E`) and passes under both Debug and Release, captured as JUnit XML. Evidence: `verify/runs/establish-architecture-foundation/20260731-043713/artifacts/windows-build-validation/{debug,release}-junit.xml`. End-to-end fuzzing of the live named pipes (rather than the shared parsing code in-process) remains a further follow-up once Tier-1 IPC validation lands.
