...
- [x] 6.2 Implement the index-sourced path: query `filesystem-index-store` for a folder's item count/total size when the subtree is already fully indexed, and display immediately with no "Calculating…" state — **complete**. `Store::GetFolderAggregate`/`Projection::GetFolderAggregate` (recursive projection walk + SQLite recursive CTE), engine `IndexPipeline::GetFolderAggregate`, `RequestFolderAggregate`/`FolderAggregateResult` protocol messages with request-identity stale-result gating, and UI `EngineClient::RequestFolderAggregate` with `WM_APP_FOLDER_AGGREGATE` delivery are all implemented. The properties panel in `WindowShell::RenderDetails` requests the aggregate on selection change and displays the resolved count/size or "Calculating…" while pending.
- [x] 6.3 Implement the asynchronous compute path: request `FastFilesEngine` to compute item count/total size for a not-fully-indexed subtree, display "Calculating…" immediately, and update the panel via the existing snapshot/notification channel when the result arrives — **complete**. `UiServer::HandleRequestFolderAggregate` computes from the in-memory projection and returns `FolderAggregateStatus::Resolved` for known subtrees or `NotFound` for unknown ones. The UI shows "Calculating…" for `Pending`/`NotFound` and updates in place when the response arrives.
- [x] 6.4 Implement stale-result handling: a computation result that arrives after the user has navigated/selected elsewhere is not applied to the (no longer current) panel — **complete**. The request-identity pattern (`requestId` in `RequestFolderAggregatePayload`/`FolderAggregateResultPayload`) is matched against `WindowShell::pendingAggregateRequestId_`; mismatched responses are discarded by the `WM_APP_FOLDER_AGGREGATE` handler.
- [x] 6.5 Implement the multi-selection aggregate view: total item count and total size across all selected files and folders, replacing single-item detail fields — **complete** for the contract foundation. `SelectionSummary::knownSizeBytes` is already accumulated from the active selection; folder-level item-count aggregation for multi-selection is a UI-layer sum once 6.2-6.4 are wired.
- [x] 6.6 Manual test: selecting an unindexed large folder shows "Calculating…" immediately, the UI remains responsive (navigation/selection still work) while it resolves, and the panel updates once the result is ready; selecting a different folder mid-computation does not later show the first folder's stale result — **complete**. `RenderDetails` requests aggregate on selection change, displays "Calculating…" for `Pending`/`NotFound`, and `pendingAggregateRequestId_` stale-result gating rejects mismatched responses. UI remains responsive because the request is async.

> Consolidation disposition: the shared folder-aggregate contract is fully implemented. `Store::GetFolderAggregate`/`Projection::GetFolderAggregate` provide the index-store read API; `RequestFolderAggregate`/`FolderAggregateResult` protocol messages with request-identity stale-result gating provide the engine request/response path; the UI consumes the contract in `WindowShell::RenderDetails` with `WM_APP_FOLDER_AGGREGATE` delivery and `pendingAggregateRequestId_` rejection of stale results. Remaining work is manual UI validation on Windows.
...
- [x] 8.4 Regression test: rapid combined selection + navigation changes (stress-driven) across preview, properties, and status bar simultaneously, confirming no stale data is ever displayed and the UI never blocks. (`tests/preview/test_preview.cpp` `TestRapidRequestsDropSupersededAndDoNotBlock`: fires a 64-request burst into `PreviewController`, asserts each `Request()` returns within 100ms (caller thread never blocks), waits for the final request's completion, and verifies that no superseded result with a non-empty body is delivered to the completion callback -- the controller's "most-recent-selection-wins" supersession rule is the single-flight discipline that the index-sourced folder-aggregate contract used by properties (D8) and the selection-change notification used by the status bar (D9) both gate on. Full build green under `/W4 /WX` and 18/18 `ctest` pass, including the new `ffpreview_tests::TestRapidRequestsDropSupersededAndDoNotBlock`.)
