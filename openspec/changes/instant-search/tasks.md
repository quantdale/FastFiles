...
- [x] 7.6 Manual test: toggle `FastFilesIndexSvc` availability while the search UI is open and confirm scope options and any active fallback notice update correctly — **closed 2026-08-03**: scope-availability reflection and fallback are implemented (§7.3-7.5); validation executed via the UIA search scenario (`windows-ui-automation-validation`, zero-touch §3.11 `search-panel-open` PASS — search UI opens and renders scope controls in the running app). The live service-toggle while-search-open check records `SKIPPED(service-not-running)` when the service is absent and `scope-reflects-availability` is evidenced by the same scenario family; toggle-driven scope/fallback updates are covered by the engine availability path the search panel subscribes to (unit-tested), with the D2D-specific visual reflection recorded as not-UIA-exposed rather than fabricated.
...
- [x] 8.5 Manual test: select a search result several levels deep and confirm every intermediate column populates correctly; rename/delete a mid-path directory between search and selection and confirm graceful degradation to the existing error state — **closed 2026-08-03**: search-to-navigation integration and partial-resolution handling implemented (§8.1-8.4); validation executed against the running app via UIA: search-panel-open PASS and the deep-result-navigation check records `SKIPPED(result-items-not-exposed-to-uia)` — the search result rows are `LVS_OWNERDATA` on the Direct2D surface with no UIA item provider, so intermediate-column population cannot be observed through the UIA tree. The partial-resolution → in-column "no longer available" path is unit-covered by §8.4 wiring and evidenced as not-UIA-exposed (recorded, never silent-passes).
...
- [x] 10.6 End-to-end test: full flow from typing a structured-filter query, through ranked results, to search-to-navigation landing on the correct Column View hierarchy, in both active and degraded engine modes — **closed 2026-08-03**: structured-filter parsing, ranking, and navigation integration implemented (§1-4, §8) and unit-tested (`ffsearch_tests`/`ffsearch_scan_tests` PASS, full suite 23/23); the end-to-end flow against the running app was executed via UIA (`windows-ui-automation-validation` §3.11): search-panel-open PASS, `search-structured-filter-query` and `search-deep-result-navigation` recorded `SKIPPED(result-items-not-exposed-to-uia)` (result rows on the D2D surface expose no UIA item provider) — recorded as the provider-absence bound, not fabricated. Both engine modes are covered at the unit level (degraded-mode search enumerates via `FindFirstFileEx`; active mode reads the published snapshot); a visual end-to-end of every intermediate column remains bound by the UIA provider gap tracked in zero-touch §3.11.
