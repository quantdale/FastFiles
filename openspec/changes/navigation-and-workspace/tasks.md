...
- [x] 1.4 Verify two or more concurrently live `NavigationContext` instances reading different paths produce correct, independent results with zero additional IPC round-trips beyond the shared snapshot's existing notification mechanism — **complete**: `TestConcurrentIndependentContextsReadDifferentPaths` in `tests/navigation/test_navigation_workspace.cpp` holds three contexts live simultaneously (tab 0 dual-pane on `C:\`/`D:\`, tab 1 on `E:\`), each with its own path/history/breadcrumbs/columns, and verifies interleaved back/forward navigation never bleeds state across contexts (paths, histories, and breadcrumbs all independent after switches). Zero-IPC property verified by static audit: `NavigationWorkspace.cpp` contains no pipe, mapped-file, or window-message primitive (its only I/O is the workspace-state settings file); all reads go through the shared in-process index snapshot per the header contract "Plain UI state: no IPC/engine handles", so per-context reads add zero round-trips.
