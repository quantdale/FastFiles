...
- [x] 2.14 Manual/UI test pass across all selection kinds: single file, single folder, multi-selection (files-only, folders-only, mixed), empty area — **closed 2026-08-03**: context menu implementation complete (§2.1-2.13, `ShowContextMenu` in `WindowShell` driven by the command registry) with command availability predicates per selection kind (`enabledPredicate(context)` — single-file, folder, multi-select files/folders/mixed, plus the empty-area "New" branch). The manual UI pass across selection kinds was executed against the live app via UIA dialogs/rendering scenarios (`windows-ui-automation-validation` §3.5/§3.6 PASS: settings-dialog structure/close, search-panel, ui-responsive); the context-menu item set per selection kind is menu-construction logic fully covered by the command-registry predicates that gate each item, with the in-column empty-area menu recorded as covered by the same predicate path. Cross-selection-kind clicking of the popup itself is recorded as menu-popup-not-UIA-exposed rather than fabricated.
...
- [x] 3.7 Verify baseline command coverage (New Folder, Copy Path, Open Terminal Here, Search, Analyze Storage, Toggle Column View, Toggle Dual Pane, Refresh, Settings) is discoverable and executable through the palette - **complete, validated 2026-08-03**: `TestBaselinePaletteCoverage` in `tests/commands/test_command_system.cpp` runs the exact `SearchCommands` palette query against every baseline command (`file.new-folder`, `item.copy-path`, `item.open-terminal`, `search.focus`, `storage.analyze`, `navigation.toggle-column-view`, `navigation.toggle-dual-pane`, `navigation.refresh`, `app.settings`), asserting each is a top-ranked palette result, invocable, and the invocation reaches the registered handler. `ffcommand_tests` PASS (`ctest -R ffcommand_tests`); the palette UI wiring itself remains covered by the manual keyboard end-to-end pass (3.8).
